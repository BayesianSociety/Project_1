...
- frontend and backend must be implemented in TypeScript using Next.js 16.x only, staying on the current Active LTS release
- rendering must use WebGL 2.0 for the gameplay experience and scene presentation
...
- persistence must use SQLite 3.52.x only, staying on the latest stable patch release
...
- cartoon-style
- polished and beautiful rather than placeholder-like
...
- original characters, pets, props, and environments
- expressive pet animations and readable character poses
...
- the player character, pets, obstacles, items, and destinations must remain visually clear at all times
...
Visual asset creation for the project will be provided externally rather than generated by the implementation agent.
...
- placeholders may be used temporarily for development
...
Per-puzzle organization should be used so ownership and lookup remain clear:

- each puzzle should have its own subdirectory such as `puzzle-01`, `puzzle-02`, and so on
- background, sprite, and layout-reference files for a puzzle should use matching puzzle identifiers where practical

Example structure:

- `public/assets/backgrounds/puzzle-01/clinic-room.png`
- `public/assets/sprites/puzzle-01/vet-helper.png`
- `public/assets/sprites/puzzle-01/cat-patient.png`
- `public/assets/sprites/puzzle-01/brush.png`
